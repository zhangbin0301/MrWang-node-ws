...
const UUID = process.env.UUID || '248a6c09-efde-4ce7-b415-4092a78cc5d2'; // 运行哪吒v1,在不同的平台需要改UUID,否则会被覆盖
const NEZHA_SERVER = process.env.NEZHA_SERVER || 'nazhav1.gamesover.eu.org:443';       // 哪吒v1填写形式：nz.abc.com:8008   哪吒v0填写形式：nz.abc.com
const NEZHA_PORT = process.env.NEZHA_PORT || '';           // 哪吒v1没有此变量，v0的agent端口为{443,8443,2096,2087,2083,2053}其中之一时开启tls
const NEZHA_KEY = process.env.NEZHA_KEY || 'qL7B61misbNGiLMBDxXJSBztCna5Vwsy';             // v1的NZ_CLIENT_SECRET或v0的agent端口                
const DOMAIN = process.env.DOMAIN || 'pepaapig-pig.hf.space';       // 填写项目域名或已反代的域名，不带前缀，建议填已反代的域名
...
const NAME = process.env.NAME || 'US-Huggingface';         // 节点名称
...
const httpServer = http.createServer((req, res) => {
  if (req.url === '/') {
    ...
    const vlessURL = `vless://${UUID}@${DOMAIN}:443?encryption=none&security=tls&sni=${DOMAIN}&fp=chrome&type=ws&host=${DOMAIN}&path=%2F${WSPATH}#${ISP}-${NAME}`;
    const trojanURL = `trojan://${UUID}@${DOMAIN}:443?security=tls&sni=${DOMAIN}&fp=chrome&type=ws&host=${DOMAIN}&path=%2F${WSPATH}#${ISP}-${NAME}`;
